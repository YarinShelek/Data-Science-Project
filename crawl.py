...
try:
    ...
    while True:
        ...
        players_list = driver.find_elements(By.CLASS_NAME, "summoner")
        ads = driver.find_elements(By.ID, "desktop-anchor-close")
        if len(ads)>0: #close ads
            for k in range(len(ads)):
                ads[k].click()
        players_list[i].click() #enter player page
        time.sleep(5)
        players_stats = {}
        #ranking
        ...
        #stats page
        stats = driver.find_elements(By.CLASS_NAME, "nav-tab-link") #get to stats page
        stats[1].click()
        ...
        if i >= len(players_list):
            ...
            i = -1
            time.sleep(6)
        stop = driver.find_elements(By.CLASS_NAME, "content-section leaderboard_table_error")
        if len(stop) > 0:
            break
